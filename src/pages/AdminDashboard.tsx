        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 md:gap-8">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 flex flex-col">
              ...
              {paths.map((p, index) => (
                <div key={p.id} className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className={`flex-1 text-right p-3 rounded-lg border transition-colors text-sm ${selectedPath === p.id ? 'bg-emerald-50 border-emerald-500 text-emerald-700 font-bold' : 'bg-slate-50 border-slate-200'}`}
                  >
                    {p.title}
                  </button>
                  <div className="flex flex-col gap-1">
                    <button onClick={() => setEditingPath(p)} className="p-1.5 text-slate-400 hover:text-violet-600"><Edit3 className="w-3.5 h-3.5" /></button>
                    <button onClick={() => handleDeletePath(p.id)} className="p-1.5 text-slate-400 hover:text-red-600"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              ))}
            </div>
            <div className="space-y-3 pt-6 border-t border-slate-100">
              ...
              />
              <button onClick={handleAddPath} className="w-full bg-emerald-500 text-white py-2 rounded-lg font-bold hover:bg-emerald-600 transition-colors text-xs">إضافة مسار</button>
            </div>
          </div>
              ...
              <>
                ...
                <div className="space-y-3 pt-6 border-t border-slate-100">
                  ...
                  <button 
                    ...
                    className="w-full bg-slate-100 text-slate-600 py-2 rounded-lg font-bold hover:bg-slate-200 mb-2 transition-colors flex items-center justify-center gap-2 text-xs"
                  >
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200">
            ...
            {selectedLesson ? (
              <div className="space-y-4">
                <p className="text-xs text-slate-500">يحتوي هذا الدرس على {pages.length} صفحات.</p>
                <button 
                  onClick={() => {
                    const lesson = lessons.find(l => l.id === selectedLesson);
                    if (lesson && selectedPath) setEditingLesson({ ...lesson, path_id: selectedPath });
                  }}
                  className="w-full bg-violet-600 text-white py-3 rounded-xl font-bold hover:bg-violet-700 shadow-lg shadow-violet-100 flex items-center justify-center gap-2 transition-all text-sm"
                >
                ...
                <div className="space-y-2 mt-6">
                  {pages.slice(0, 5).map((p, i) => (
                    <div key={p.id} className="p-2 bg-slate-50 border border-slate-100 rounded-lg text-[10px] flex items-center gap-2">
            ...
            ) : <p className="text-slate-400 text-center py-12 text-sm">اختر درساً أولاً</p>}
          </div>
        </div>
      )}
